...
WAVE_MODLE_2D* pad_abs(WAVE_MODLE_2D* mode, float dt);

float get_time_step_size(WAVE_MODLE_2D* model)
{
    float max_velocity = 0;
    for (int i = 0; i < model->nz*model->nx; ++i)
        max_velocity = fmaxf(max_velocity, model->velocity[i]);

    return COURANT_NUMBER*model->dx/max_velocity;
}
...
int propagate(WAVE_MODLE_2D* model, float* result)
{
    float max_q = 0;
    for (int i = 0; i < model->nz*model->nx; ++i)
        max_q = fmaxf(max_q, model->abs_model[i]);
    int nx = model->nx;
    float dt = model->dt;
    ...
    for (int step = 0; step < total_steps; ++step)
    {
        ...
        memset(u_next+2*nx, 0 ,nx*sizeof(float));
        memcpy(result+step*(nx-2*model->pad_num), u_next+model->receiver_depth*nx+model->pad_num, (nx-2*model->pad_num)*sizeof(float));
    }
    free (src);
    free(u_curr);
    free(u_prev);
